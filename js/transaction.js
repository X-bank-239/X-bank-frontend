...
class TransactionsService {
    ...
    renderAccounts(accounts) {
        ...
        accountsGrid.innerHTML = accounts.map(account => `
            <div class="account-card ${account.type}">
                ...
                <div class="account-balance">${account.balance.toLocaleString('ru-RU')} ₽</div>
                <div class="account-number">${this.maskAccountNumber(account.number)}</div>
            </div>
        `).join('');
    }
            ...
            return `
                <div class="detailed-transaction-item">
                    <div class="transaction-info">
                        <div class="transaction-icon ${amountClass}">${this.getTransactionIcon(transaction)}</div>
                        <div class="transaction-details">
                            ...
                            <p>${new Date(transaction.date).toLocaleDateString('ru-RU')} • ${transaction.accountNumber ? 'Карта • ' + this.maskAccountNumber(transaction.accountNumber) : 'Перевод'}</p>
                        </div>
                    </div>
                    <div class="transaction-amount ${amountClass}">
                        ${amountSign} ${absoluteAmount.toLocaleString('ru-RU')} ₽
                    </div>
                </div>
            `;
        } else {
    ...
    updateStatistics(transactions) {
        ...
        if (incomeElement) incomeElement.textContent = `+ ${income.toLocaleString('ru-RU')} ₽`;
        if (expenseElement) expenseElement.textContent = `- ${expense.toLocaleString('ru-RU')} ₽`;
    ...
    async processTransfer(formData) {
        ...
        try {
            ...
            showSuccess(`Перевод на сумму ${formData.amount.toLocaleString('ru-RU')} ₽ выполнен успешно!`);
